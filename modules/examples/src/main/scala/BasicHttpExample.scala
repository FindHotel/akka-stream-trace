    ...
    path("frontend") {
      get {
        Directives.extractTraceId { t_id =>
          ...
          println(t_id)
          val req = HttpRequest(uri = s"http://localhost:8080/backend?i=$rand").withHeaders(t_id.get, RawHeader(Directives.HTRACE_HEADER_NAME, t_id.get.value), RawHeader("foo", "bar"))
